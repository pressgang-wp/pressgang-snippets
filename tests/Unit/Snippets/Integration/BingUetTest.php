...
use PressGang\Snippets\Integration\BingUet;
 ...
 * parsing issue.
 */
class TestableBingUet extends BingUet {
...
/**
 * @covers \PressGang\Snippets\Integration\BingUet
 */
class BingUetTest extends TestCase {
	...
	public function test_script_renders_when_id_set_and_user_allowed(): void {
		$snippet = new TestableBingUet( [] );

		Functions\expect( 'get_theme_mod' )
			->once()
			->with( 'bing-track-logged-in' )
			->andReturn( 0 );
		...
		$this->assertSame( 'snippets/integration/bing-uet.twig', $snippet->rendered[0]['template'] );
		$this->assertSame( '12345678', $snippet->rendered[0]['context']['bing_uet_id'] );
	}
