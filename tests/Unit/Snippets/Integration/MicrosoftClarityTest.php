...
use PressGang\Snippets\Integration\MicrosoftClarity;
 ...
 * parsing issue.
 */
class TestableMicrosoftClarity extends MicrosoftClarity {
...
/**
 * @covers \PressGang\Snippets\Integration\MicrosoftClarity
 */
class MicrosoftClarityTest extends TestCase {
	...
	public function test_script_renders_when_id_set_and_user_allowed(): void {
		$snippet = new TestableMicrosoftClarity( [] );

		Functions\expect( 'get_theme_mod' )
			->once()
			->with( 'clarity-track-logged-in' )
			->andReturn( 0 );
		...
		Functions\expect( 'get_theme_mod' )
			->once()
			->with( 'clarity-id' )
			->andReturn( 'clarity123' );

		$snippet->script();

		$this->assertCount( 1, $snippet->rendered );
		$this->assertSame( 'snippets/integration/microsoft-clarity.twig', $snippet->rendered[0]['template'] );
		$this->assertSame( 'clarity123', $snippet->rendered[0]['context']['clarity_id'] );
	}
