...
use function PressGang\Snippets\update_post_meta;
use function PressGang\Snippets\woocommerce_wp_text_input;
use function PressGang\Snippets\wp_date;
...
class WooCommerceBackorders {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_process_product_meta',
			[ $this, 'woocommerce_product_custom_fields_save' ] );

		// Message display
		add_filter( 'woocommerce_out_of_stock_message',
			[ $this, 'out_of_stock_message' ] );
		add_filter( 'woocommerce_get_availability_text',
			[ $this, 'availability_backorder_text' ], 10, 2 );
		add_filter( 'woocommerce_composited_product_availability',
			[ $this, 'availability_backorder_text' ], 10, 2 );
	...
	public function out_of_stock_message( $text ) {
		...
		if ( $product ) {
			if ( $backorder_date = get_field( 'backorder_date',
				$product->get_id() ) ) {
				$backorder_date = wp_date( get_option( 'date_format' ),
					strtotime( $backorder_date ) );
				$text           = sprintf( __( "Out of stock. Expected delivery date %s.",
					THEMENAME ), $backorder_date );
			}
		}
				...
				case 'onbackorder' :
					if ( $backorder_date = get_field( 'backorder_date',
						...
						$backorder_date = wp_date( get_option( 'date_format' ),
							strtotime( $backorder_date ) );
						$availability   =
							sprintf( __( "Available on backorder. Expected delivery date %s.",
								THEMENAME ), $backorder_date );
					}
					break;
				case 'outofstock' :
	...
	public function woocommerce_product_custom_fields_save( $post_id ) {
		...
		if ( ! empty( $backorder_date ) ) {
			update_post_meta( $post_id, 'backorder_date',
				esc_attr( $backorder_date ) );
		}
	}
