...
use function PressGang\Snippets\wp_redirect;
...
class WooCommerceTaxToggle {
	...
	public function set_tax_display() {
		if ( isset( $_POST[ sprintf( "%s_toggle", self::COOKIE_NAME ) ] ) ) {
			$display_tax = filter_input( INPUT_POST, self::COOKIE_NAME,
			...
			if ( wp_redirect( add_query_arg( 'vat', $display_tax,
				\Timber\UrlHelper::get_current_url() ) ) ) {
				exit;
			}
		}
	}
	...
	public function add_to_twig( $twig ) {
		$twig->addFunction( new \Twig\TwigFunction( 'tax_toggle',
			[ $this, 'render_tax_toggle' ] ) );
	...
	public function render_tax_toggle() {
		\Timber\Timber::render( 'woocommerce/tax-toggle.twig', [
	...
	protected static function set_tax_cookie( $display_tax ) {
		setcookie( 'woocommerce_tax_display', $display_tax,
			time() + 365 * 24 * 60 * 60 );
	}
