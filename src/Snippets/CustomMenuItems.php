...
class CustomMenuItems {
	...
	public function __construct() {
		if ( $items = Config::get( 'custom-menu-items' ) ) {
			foreach ( $items as $slug => &$args ) {
				self::add_subitems_to_menu( $slug,
					isset( $args['parent_object_id'] ) ? $args['parent_object_id'] : 0,
					$args['subitems'] );
			}
		}
	}
	 ...
	 *     'url' => '/bar') )
	 */
	public static function add_subitems_to_menu(
		$menu_name,
		$parent_object_id = 0,
		$subitems = []
	) {
		...
		use ( $menu_name, $parent_object_id, $subitems ) {
			...
			foreach ( $subitems as $subitem ) {
				...
				$items[] = (object) [
					'ID'               => $menu_order + 1000000000,
					// ID that WP won't use
					'title'            => $subitem['text'],
					'url'              => $subitem['url'],
